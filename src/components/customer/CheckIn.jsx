...
const CheckIn = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        } else if ((found.status || 'confirmed').toLowerCase() !== 'confirmed') {
  ...
  const handleCheckIn = () => {
    // No backend endpoint defined; simulate local confirmation
    setCheckedIn(true);
  };
        ...
        <div className="bg-white rounded-xl shadow ring-1 ring-black/5 overflow-hidden">
          ...
          <div className="p-6 space-y-6">
                ...
                <ul className="divide-y divide-gray-100">
                  {passengers.map((p, idx) => (
                    <li key={idx} className="py-3 flex items-center justify-between">
                      ...
                      <div className="text-sm text-gray-700">Seat: {(Array.isArray(booking?.seats) && booking.seats[idx]) || 'TBD at gate'}</div>
                    </li>
                  ))}
                </ul>
              )}
            ...
            <div className="pt-2 flex justify-end gap-3">
              {!checkedIn ? (
                <button onClick={handleCheckIn} className="px-4 py-2 rounded-md text-white text-sm font-semibold bg-gradient-to-r from-indigo-600 to-fuchsia-600 hover:from-indigo-700 hover:to-fuchsia-700">
                  Confirm Check-in
                </button>
              ) : (
                <button onClick={() => navigate(`/customer/bookings/${bookingId}`)} className="px-4 py-2 rounded-md text-sm font-semibold bg-white ring-1 ring-black/5 shadow hover:bg-gray-50">
